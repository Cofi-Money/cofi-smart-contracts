...
contract TokenERC20Facet is IERC20, Modifiers {
    ...
    function balanceOf(address _account) public view returns (uint256) {
        return LibToken._balanceOf(_account);
    }
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        require(_to != address(0), 'ERC20Facet: Transfer to zero address');
        require(s.paused < 1, 'ERC20Facet: Token paused');
        require(_value <= balanceOf(msg.sender), 'ERC20Facet: Transfer greater than balance');
        require(s.frozen[msg.sender] < 1, 'ERC20Facet: Caller is frozen');
        require(s.frozen[_to] < 1, 'ERC20Facet: Recipient account is frozen');

        LibToken._executeTransfer(msg.sender, _to, _value);
    ...
    function transferFrom(
        ...
        LibToken._transferFrom(_from, _to, _value);
    ...
    function redeem(
        ...
        LibToken._executeTransfer(_from, _to, _value);
    ...
    function increaseAllowance(
        ...
        LibToken._increaseAllowance(_spender, _addedValue);
    ...
    function decreaseAllowance(
        ...
        LibToken._decreaseAllowance(_spender, _subtractedValue);
    ...
    function mint(address _account, uint256 _amount) external onlyApp returns (bool) {
        ...
        require(s.frozen[_account] < 1, 'ERC20Facet: Recipient account is frozen');
        LibToken._mint(_account, _amount);
    ...
    function mintOptIn(address _account, uint256 _amount) external onlyApp returns (bool) {
        ...
        if (LibToken._isNonRebasingAccount(_account)) {
    ...
    function burn(address _account, uint256 _amount) external returns (bool) {
        ...
        require(s.paused < 1, 'ERC20Facet: Token paused');
        LibToken._burn(_account, _amount);
